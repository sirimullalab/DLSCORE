###################################################################################
# This is a modified version of nnscore.py                                        #
# 																											 #
# CHANGES:																							    #
...
#     of nnscore.py																					 #
#																											 #
#  Necessary files to run: networks.pickle, model.json, model.h5, std.pickle      #
#																											 #
...
######## IMPORT MODULES NECESSARY FOR KERAS AND DLSCORE MODEL IMPLEMENTATION ############
import keras
...
def calculate_score(lig, rec, cmd_params, actual_filename_if_lig_is_list="", actual_filename_if_rec_is_list="", line_header = ""):
        ...
        #Save Binana Descriptors (this was added, is extra from the lab)
        #f=open('binana_descriptors.csv','ab')
        #numpy.savetxt(f,numpy.array(actual_filename_if_lig_is_list[0:4],dtype=str).reshape(1,), fmt="%s")
        #numpy.savetxt(f,descriptors[None],delimiter=',')
        #f.close()
        #f = open("input_vector.pickle", "wb")
        ##pickle.dump(d, f)
        #f.close()
...
#sys.exit()   
        ...
        for net_array in nets:
                try:
                    ...
                    print (line_header + "Network #" + str(len(scores) + 1) + " gave a score of " + str( np.round(val,3)) + " (" + score_to_kd(val) + ")")
                    scores.append(val)
                    total = total + val
        ...
        try:
                ...
                if not os.path.isdir('output'):
                    os.mkdir('output')
                with open('output/' + pdb_id + '_predicted_values.csv', 'a') as f:
        ...
        if len(scores) != 0:
                ...
                print (line_header + "Best Score:         ", np.round(best_score,3), "(" + score_to_kd(best_score) + ")")
                print (line_header + "Average Score:      ", np.round(average,3), "(" + score_to_kd(average) + ")")
...
for score in best_scores:
    print (str(count).center(6," ") + "|" + score[5].center(10," ") + "|" + str(np.round(score[2],3)).center(15," ") + "|" + score_to_kd(score[2]).replace("Kd = ","").center(14," ") + "|" + ("#" + str(score[3])).center(9," "))
    count = count + 1
...
for score in average_scores:
    print (str(count).center(6," ") + "|" + score[5].center(10," ") + "|" + str(np.round(score[0],3)).center(15," ") + "|" + str(np.round(score[1],3)).center(20," ") + "|" + score_to_kd(score[0]).replace("Kd = ","").center(15," "))
    count = count + 1
...
for best_network_score in best_network_scores:
    print (textwrap.fill("Best pose scored by network #" + str(count) + ": " + best_network_score[1] + " (Score = " + str(np.round(best_network_score[0],3)) + " = " + score_to_kd(best_network_score[0]).replace("Kd = ","") + ")"))
    count = count + 1
